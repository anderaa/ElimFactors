...
paramCombos <- expand.grid(initialPopSizeList, fecundityFactorList, 
                           ...
                           survivalProbList, initialFracImmuneList, inMigrationFactor)
...
#simulationData <- foreach(c = 1:5, .combine=rbind, .inorder=FALSE) %dopar% {
  ...
  monthsOfPressure      <- 3     # USER CHOICE
  dogsPerMonthExposed   <- ceiling((initialPopSize * 0.002 * 10 / 4) / 3)  # USER CHOICE
  timeLimitExposed      <- 22
...
# but also examine pNoDogsLeft, sometimes population could have been wiped out
#colnames(simulationData) <- c('qElim1', 'qElim2', 'qElim3', 'qElim4', 'qElim5', 
#                              'qExtinct1', 'qExtinct2', 'qExtinct3', 'qExtinct4', 'qExtinct5',
#                              'qElimWithDogs1', 'qElimWithDogs2', 'qElimWithDogs3', 'qElimWithDogs4', 'qElimWithDogs5',
